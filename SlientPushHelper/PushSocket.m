...
#define PUSH_DISPATCH_QUEUE_SKT_NAME "com.cdfortis.push.sockect"
...
-(instancetype)init{
    if(self = [super init]){
        _isRun = _isStart = NO;
        //1.异步socket委托处理线程队列
        dispatch_queue_t dq = dispatch_queue_create(PUSH_DISPATCH_QUEUE_SKT_NAME, NULL);
        //2.socket异步处理初始化
...
-(void)start{
    ...
    if([_socket isDisconnected]){
        ...
        //连接服务器
        NSError *err = nil;
        PushSocketSetting *cfg = conf.socket;
...
-(void)addOrChangedTagHandler{
    ...
    //验证配置
    if(!conf || !conf.alias || !conf.alias.length){
        [self throwsErrorWithMessageType:PushSocketMessageTypeSubscribe andMessage:@"获取用户alias数据失败!"];
        return;
    }
    _config = conf;//替换
...
#pragma mark -- 内部方法
//获取配置
-(BOOL)loadWithConfig:(PushServiceData **)cfg{
...
-(void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag{
    ...
    //设置时间戳
    _lastPingTime = [NSDate date].timeIntervalSince1970;
    //解码处理
